...
#     debug(Sc)
    ...
    for i,S in enumerate(Sc.splines) :
        ...
        print S
        if show : S.plot(plt,titre='spline-%d'%i, numbers='c')
    if show : Sc.plot(plt, numbers=['3c'])
    Sc.plotCourbure()
...
#                                     ('cubic','not-a-knot')
    ...
    if show : S.plot(plt, titre='Original '+str(S.methode))

    S.hardScale((0.2,0.2), centre=np.asarray((0,0)))
    debug(S)
    if show : S.plot(plt, titre='S.hardScale((0.2,0.2))')

    S.hardRotate(10)
    debug(S)
    if show : S.plot(plt, titre='S.hardRotate(10)')
    ...
    debug(join_1=S,R=R)
    if show : S.plot(plt, titre='S = NSplineComposee(**S.toDump())')

    dump = S.toDump()
    S.load(dump)
    R = S.cpoints[S.ruptures]
    debug(join_1=S,R=R)
    if show : S.plot(plt, titre='S.load(S.toDump())')

    S.join(1)
    R = S.cpoints[S.ruptures]
    debug(join_1=S,R=R)
    if show : S.plot(plt, titre='join(1)'+str(S.methode))
...
#             ['cubic',((1, 20, 0), (1, 1, 0))]#intrados
    ...
    if show : S.plot(plt, titre='S.split(40)'+str(S.methode))
#     numfig += 1
#     plt.figure(numfig)
...
#     S.plot(plt, titre='translate((100,0))')
    ...
    try : S.removePoint(npt)
        ...
        rdebug(msg)
    if show : S.plot(plt, titre='removePoint(%d)'%npt)
#     S[4] = (0,0.8)
    try :
        pt = (20,20)
        S.insertPoint(pt)
        if show : S.plot(plt, titre='insertPoint(%s)'%str(pt))
    except ValueError as msg :
        print msg
    debug(points=S.cpoints.tolist())
    i = len(S)/2
    S.insertPoint(pt,i)
    if show : S.plot(plt, titre='insertPoint(%s,%d)'%(pt,i))
    if show : S.plot(plt, titre='S[4] = (0,0.8)')
    S[0] = (2,0.5)
    if show : S.plot(plt, titre='S[0] = (2,0.5)')
    S[len(S)-1] = S[0]
    if show : S.plot(plt, titre='S[len(S)-1] = S[0]')
#     return
    S.appendPoint((2,1.5))
#     print S
#     print S.cpoints
    if show : S.plot(plt, titre='S.appendPoint((2,1.5))')
    S.insertPoint((2,-2))
    if show : S.plot(plt, titre='S.insertPoint((2,-2))')
    S.insertPoint((6,0))
    print S
    if show : S.plot(plt, titre='S.insertPoint((6,0))')
    S.removePoint(3)
    if show : S.plot(plt, titre='S.removePoint(3)')
...
#         S1.plot(plt)
        ...
        debug(S1)
        if show : S1.plot(plt,titre=u'élaguage')
...
#     debug(original=S,R=R)
    ...
    npt = S.ruptures[1]
    try : S.removePoint(npt)
    except NotImplementedError as msg :debug(msg)
    debug(S)
    if show : S.plot(plt, titre='removePoint(%d)'%npt)
...
#     return
    ...
    S._update()
    debug(S)
    if show : S.plot(plt, titre='S._update()')
    rdebug('**********************\ndouteux, a verifier\n**********************')
...
def testMain(show=False):
    ...
    for k,filename in enumerate(files[:]) :
        if 0:debugNSplineComposee(filename, show=show)#k in [6,7])
        if 0:testNSplineComposee(filename, show=show)#k in [6,7])
    if 0 : testParticular(files[6], show=show)
